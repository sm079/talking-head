...
class SrcDstDataset(IterableDataset):
    ...
    def transform(self, image, mask, rotation_range, scale_range, random_flip, random_hue):
        ...
        # Center Crop
        crop_size = 480 + random.randint(int(-0.15 * self.image_size), 0)
...
def main():
    ...
    parser.add_argument("-c", "--config", default="config1.yaml", help="Path to the YAML config file")
    ...
    interab_path = config["model_paths"]["interab"]
    inter_b_path = config["model_paths"]["inter_b"]
    ...
    inter_in_features  = 1 * E_DIMS * 8 * ( IN_IMAGE_SIZE // 32) ** 2
    inter_out_features = 2 * E_DIMS * 8 * (OUT_IMAGE_SIZE // 32) ** 2

    model_path = MODELS_DIR/f"encoder_{steps//1000:03d}.pth"
    if (model_path).exists(): encoder_path = str(model_path)
    model_path = MODELS_DIR/f"decoder_{steps//1000:03d}.pth"
    if (model_path).exists(): decoder_path = str(model_path)
    model_path = MODELS_DIR/f"interAB_{steps//1000:03d}.pth"
    if (model_path).exists(): interab_path = str(model_path)
    model_path = MODELS_DIR/f"inter_B_{steps//1000:03d}.pth"
    if (model_path).exists(): inter_b_path = str(model_path)
    ...
    print(f"  Decoder: {decoder_path} ({ED_DTYPE})")
    print(f"  InterAB: {interab_path} ({INTER_DTYPE})")
    print(f"  Inter B: {inter_b_path} ({INTER_DTYPE})")
    ...
    DECODER = torch.load(decoder_path).to(device=DEVICE, dtype=ED_DTYPE) if decoder_path else \
        Decoder(AE_DIMS * 4, D_DIMS, M_DIMS).to(device=DEVICE, dtype=ED_DTYPE)
    DECODER = torch.compile(DECODER, backend="eager")

    INTERAB = torch.load(interab_path).to(device=DEVICE, dtype=INTER_DTYPE) if interab_path else \
    ...
    INTERAB = torch.compile(INTERAB, backend="eager")
    
    INTER_B = torch.load(inter_b_path).to(device=DEVICE, dtype=INTER_DTYPE) if inter_b_path else \
        nn.Sequential(nn.Linear(inter_in_features, AE_DIMS), nn.Linear(AE_DIMS, inter_out_features)).to(device=DEVICE, dtype=INTER_DTYPE)
    INTER_B = torch.compile(INTER_B, backend="eager")
    ...
    while steps < total_steps:
        for src_image, src_mask, dst_image, dst_mask in loader:
            ...
            src_code = ENCODER(src_in).to(dtype=INTER_DTYPE)
            dst_code = ENCODER(dst_in).to(dtype=INTER_DTYPE)

            src_interAB_code = INTERAB(src_code).view(-1, E_DIMS * 8 * 2, OUT_IMAGE_SIZE//32, OUT_IMAGE_SIZE//32).to(dtype=ED_DTYPE)
            dst_interAB_code = INTERAB(dst_code).view(-1, E_DIMS * 8 * 2, OUT_IMAGE_SIZE//32, OUT_IMAGE_SIZE//32).to(dtype=ED_DTYPE)
            dst_inter_B_code = INTER_B(dst_code).view(-1, E_DIMS * 8 * 2, OUT_IMAGE_SIZE//32, OUT_IMAGE_SIZE//32).to(dtype=ED_DTYPE)

            src_image_pred, src_mask_pred = DECODER(torch.concat((src_interAB_code, src_interAB_code), axis=1))
            dst_image_pred, dst_mask_pred = DECODER(torch.concat((dst_inter_B_code, dst_interAB_code), axis=1))
            ...
            if steps % 100 == 0:
                with torch.no_grad():
                    ...
                    src_preview_pred, _ = DECODER(torch.concat((src_interAB_code, src_interAB_code), axis=1))
                    dst_preview_pred, _ = DECODER(torch.concat((dst_inter_B_code, dst_interAB_code), axis=1))
                    swp_preview_pred, _ = DECODER(torch.concat((dst_interAB_code, dst_interAB_code), axis=1))
            ...
            if steps % 1000 == 0:
                ...
                torch.save(DECODER, str(MODELS_DIR/f"decoder_{steps//1000:03d}.pth"))
                torch.save(INTERAB, str(MODELS_DIR/f"interAB_{steps//1000:03d}.pth"))
                torch.save(INTER_B, str(MODELS_DIR/f"inter_B_{steps//1000:03d}.pth"))
